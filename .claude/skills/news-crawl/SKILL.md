...
| 한국주택경제신문 | `rss` | 정비사업 전문 주간지. 네이버 검색 유입 1위 매체라 RSS 전량 + tier2 |
| 주거환경신문 | `rss` | 정비사업 전문. 주 1회 일괄 갱신이라 기간 필터에 절반이 걸림(정상) |
| 서울 도시계획포털 | `rss` | `news.seoul.go.kr/citybuild/feed`. 신통기획·모아타운 확정/지정 **원문**(tier1). 같은 사업을 `[기획완료]`·`[서북권]`·`[기록영상]`으로 나눠 올려 제목 dedupe가 안 걸린다 → 다이제스트의 출처당 2건 제한으로 보완 |
...
**HTTP로 못 잡는 곳**: MTN 건설부동산은 목록이 JS 렌더다. `sources.json`의 `crawl_targets`(브라우저 크롤)와 tier2 등재(네이버 검색 유입분 가중)로 커버한다.

**0건의 두 가지 의미**: `수신 0건`(WARN)은 구조 변경·차단 의심이니 확인하라. `필터로 0건`(INFO만)은 그날 부동산 건이 없었던 것 — 경기도 보도자료는 실제로 그런 날이 있다.
