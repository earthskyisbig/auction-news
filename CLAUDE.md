...
**기관 보도자료·전문지:** `config/press.json` — 국토교통부·서울시·인천시·금융위원회 보도자료 원문과 정비사업 전문지(하우징워치·하우징헤럴드·디벨로퍼뉴스·한국주택경제신문·주거환경신문)를 순수 HTTP로 받는다(키·브라우저 불필요 → GH Actions 가능). 정부 보도자료는 전 분야라 `topic_filter`로 부동산 건만 남기고, 전문지는 전량 통과. tier1(정부)·tier2(전문지), 리포트에 `🏛공식` 배지. `press_feeds.py`의 파서는 rss/molit/fsc/incheon/naver_section 5종. **서울 도시계획포털**(`news.seoul.go.kr/citybuild/feed`, tier1)이 신속통합기획·모아타운 확정/지정 원문을 매일 올린다 — 언론 기사보다 먼저 들어오는 1차 소스. MTN은 목록이 JS 렌더라 HTTP 수집 불가 → `crawl_targets`(브라우저 크롤 전용)와 tier2 등재로 커버. 경기도는 사용자가 찾은 무인증 RSS 3종(주거정책 E004·교통 E007·보도자료) 등재 — 카테고리 피드는 날짜 태그가 없어 기사번호에서 추출하며 갱신이 느려 소스별 `max_age_days: 90` 적용.

**사업 단계 이벤트 알림:** `config/watchlist.json`의 `event_alerts` 규칙 → `event_alert.py`가 다이제스트와 **별개 메시지**로 즉시 알린다. 기본 규칙 `sti_confirm`(신통기획 확정). **사업 단위로 1회만** 울린다 — 확정 하나에 언론 6곳·서울시 3건이 붙으므로 제목에서 동(洞)을 뽑아 묶고, 발송 이력은 `alerts`에 `evt:{규칙id}`/`key:{묶음키}`로 남긴다. 단계·패턴 변경은 config만 고친다.
...
| 2026-09-22 | 재개발 특화 매체 검토: 한국주택경제신문(arunews)·주거환경신문(rcnews) RSS를 press 채널에 추가, arunews·rcnews·아유경제(areyou)·대한경제(dnews) tier2 등재 + 기존 1,030행 소급 재스코어 | config/press.json, config/sources.json | 최근 30일 재개발 카테고리 출처 감사: **arunews가 287건으로 1위인데 tier3 미등재(평균 42점)**로 45점 컷 아래 묻히고 있었음. areyou 79·dnews 109도 동일. 아파트관리신문은 주택관리(입대의·관리사) 매체라 제외, 아유경제·대한경제·땅집고·리얼캐스트는 RSS 없음(검색 유입분 티어 가중으로 커버). 검증: arunews 40건(재개발 39)·rcnews 33건 수집, 재개발 45점↑ 1,352→1,505 |
| 2026-09-23 | **서울 도시계획포털 피드 추가**(`seoul_citybuild`, tier1) — 신통기획·모아타운 확정/지정 원문. 다이제스트에 **출처당 최대 2건** 제한(`MAX_PER_SOURCE`) 추가 | config/press.json, tg_common.py | 서울시 신통기획 보도자료를 매일 아침 받고 싶다는 요청. 기존 서울시 보도자료 RSS(bbsNo=158)는 전 분야 50건이라 정비사업 원문이 밀림 — citybuild는 주택·도시계획 전용. 검증: 10건 수집 전건 tier1 55~68점, 홍제동 9-81 확정 원문 3건 진입. 다만 한 사업이 재개발 슬롯 5개 중 4개를 먹어 출처 제한을 함께 넣음(제한 후 2건) |
| 2026-09-23 | **신통기획 확정 전용 알림 신설**(`event_alert.py` + watchlist.json `event_alerts`) — 사업 단위 1회 발송. 🚨 **함께 발견·수정: 알림 '발송됨' 표시가 저장되지 않아 관심물건 알림이 매일 재발송되고 있었다** — 워크플로가 커밋을 텔레그램보다 먼저 해서 `notified=1` 쓰기가 .gz에 안 담겼음(미표시 70건 적체). 순서를 발송→재패킹→커밋으로 교정하고 적체분은 발송완료 처리 | event_alert.py(신규), config/watchlist.json, daily-news.yml | 신통기획 확정만 별도로 받고 싶다는 요청. 검증: 홍제동 9-81 확정이 기사 6건→알림 1건으로 묶여 발송, 재실행 시 0건(중복 없음) |
